...
//	}
	...
	protected void onPause() {// 如果突然电话到来，停止播放音乐
		if (MusicService.mplayer != null && MusicService.mplayer.isPlaying() && AccessTokenKeeper.readMusic(MusicPlay.this)) {
			position = MusicService.mplayer.getCurrentPosition();// 保存当前播放点
			MusicService.mplayer.stop();
			Log.i("MusicPlay", "MusicPlay---onPause--1");
		}
		Log.i("MusicPlay", "MusicPlay---onPause--0");
		super.onPause();
	}
	...
	protected void onResume() {
		...
		Log.i("MusicPlay", "MusicPlay---onResume--0");
		super.onResume();
	}
}
